...
import { category, participant, team, teamMember, tournament } from "@/server/db/schema";
...
export async function updateParticipantCategory(params: UpdateParticipantCategoryParams) {
    ...
    const updatedParticipant = await db.update(participant).set({ categoryId }).where(eq(participant.id, participantId)).returning();
...
export async function createTeam(params: CreateTeamParams) {
    const { tournamentId, name } = params;

    await assertTournamentOrganizer(tournamentId);

    const newTeam = await db.insert(team).values({ tournamentId, name }).returning();
